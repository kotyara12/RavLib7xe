...
function  FixTags(const InText: string; const TagsChar: Char = TagsDefC): string;
...
resourcestring
  rsErrTagNotFound              = 'Переменная ''%%%s%%'' не найдена в списке переменных!';
  rsErrTagUnterminatedTag       = 'Незавершенная переменная в строке ''%s''!';
  rsErrTagSymbolError           = 'Некорректный символ: ''%s''! Символ должен содержать префикс $ и код символа от 0 до 255.';

function FixTags(const InText: string; const TagsChar: Char = TagsDefC): string;
begin
  Result := StringReplace(InText, TagsChar, TagsChar + TagsChar, [rfReplaceAll]);
end;
...
begin
  ...
  begin
    ...
    else raise ERVarListError.CreateFmt(rsErrTagNotFound, [VarName]);
  end
...
begin
    ...
    begin
      ...
      begin
          ...
          begin
            ...
            begin
              ...
              begin
                OffsetS := EmptyStr;
                Inc(DatePos);
                while CharInSet(Tag[DatePos], DigChars) and (DatePos <= Length(Tag)) do
                begin
          ...
          begin
            ...
            then Rep := TagsChar + Tag + TagsChar
              ...
              try
                ...
                if RaiseError
                then raise ERVarListError.CreateFmt(rsErrTagSymbolError, [Tag])
                else Val := 32;
              end;
              Rep := Chr(Val);
              Repl := True;
        ...
        if RaiseError and not Repl then
          raise ERVarListError.CreateFmt(rsErrTagNotFound, [Tag]);
      end
      else begin
        Tag := EmptyStr;
        Tagged := True;
  ...
  begin
    if RaiseError then
      raise ERVarListError.CreateFmt(rsErrTagUnterminatedTag, [Source]);
