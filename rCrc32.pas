...
function StrCRC32(const sInStr: string): Cardinal;
function FileCRC32(const FileName: string): Cardinal;
function FileCRC32_STR(const FileName: string): string;
function FileCRC32_S0(const FileName: string): Cardinal;
function FileCRC32_T0(const FileName: string): Cardinal;
...
begin
  BufPtr := @Buf;
  crc32 := $FFFFFFFF;
  InFile := TStringStream.Create(sInStr);
...
function FileCRC32_STR(const FileName: string): string;
begin
  Result := AnsiUpperCase(Format('%.8x', [FileCRC32(FileName)]));
end;
...
begin
  ...
  then Result := FileCRC32(FileName)
  else Result := GetFileTime(FileName);
end;

function FileCRC32_T0(const FileName: string): Cardinal;
begin
  Result := FileCRC32(FileName) + Cardinal(GetFileTime(FileName));
end;
